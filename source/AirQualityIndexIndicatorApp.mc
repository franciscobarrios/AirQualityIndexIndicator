...
class AirQualityIndexIndicatorApp extends Application.AppBase {
  ...
  function onReceive(responseCode, data) {
    ...
    if (responseCode == 200) {
      ...
      Ui.switchToView(
          new AirQualityIndexIndicatorView(temp, hum, aqiUs, aqiColor, city),
  ...
  function makeRequest(lat, lon) {
    ...
    // var url= URL_BASE + "lat=-39.21703484861341&lon=-72.24387544507489" +
    // "&key=" + KEY;

    // Reykjavík 64.18513020159337, -21.728636234635115
    // var url= URL_BASE + "lat=64.18513020159337&lon=-21.728636234635115" +
    // "&key=" + KEY;

    // Mumbai 19.117931689682074, 72.89133494744027
    // var url= URL_BASE + "lat=19.117931689682074&lon=72.89133494744027" +
    // "&key=" + KEY;
